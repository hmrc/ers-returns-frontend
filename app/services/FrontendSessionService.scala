...
@Singleton
class FrontendSessionService @Inject() (
  val sessionCache: FrontendSessionsRepository,
  fileValidatorService: FileValidatorService,
  applicationConfig: ApplicationConfig
)(implicit ec: ExecutionContext)
    extends PageBuilder with Logging with Constants {

  def cache[T](key: String, body: T)(implicit
    request: RequestHeader,
    formats: json.Format[T]
  ): Future[(String, String)] =
    sessionCache.putSession[T](DataKey(key), body).recoverWith { case ex: Exception =>
      logger.error(s"Failed to cache data for key $key", ex)
      Future.failed(ex)
    }

  def remove(key: String)(implicit request: RequestHeader): Future[Unit] =
    sessionCache.deleteFromSession(DataKey(key)).recoverWith { case ex: Exception =>
  ...
  def fetchTrusteesOptionally()(implicit
    request: RequestHeader,
    formats: json.Format[TrusteeDetailsList]
  ): Future[TrusteeDetailsList] =
    fetch[TrusteeDetailsList](TRUSTEES_CACHE).recoverWith { case _: Exception =>
  ...
  def fetchPartFromTrusteeDetailsList[A](
    index: Int
  )(implicit request: RequestHeader, formats: json.Format[A]): Future[Option[A]] =
    sessionCache.getFromSession[JsValue](DataKey(TRUSTEES_CACHE)).map { optionalJson =>
      ...
      logger.info(
        s"[FrontendSessionService][fetchPartFromTrusteeDetailsList] Nothing found in cache or error in processing for index $index, which may be expected for a non-edit journey: ${ex.getMessage}"
      )
      None
    }

  def fetchOption[T](key: String, cacheId: String)(implicit
    request: RequestHeader,
    formats: json.Format[T]
  ): Future[Option[T]] =
    sessionCache.getFromSession[T](DataKey(key)).recoverWith {
      case e: NoSuchElementException =>
        logger.warn(
          s"[FrontendSessionService][fetchOption] No data found for key $key in cache $cacheId. Exception: ${e.getMessage}, timestamp: ${System.currentTimeMillis()}."
        )
        Future.successful(None)
      case ex: Throwable             =>
        logger.error(
          s"[FrontendSessionService][fetchOption] Error fetching key $key from cache $cacheId, timestamp: ${System.currentTimeMillis()}.",
          ex
        )
        Future.successful(None)
    }
  ...
  def fetchAll()(implicit request: RequestHeader): Future[CacheItem] =
    sessionCache.getAllFromSession().flatMap {
      ...
      case None            =>
        val errMsg =
          s"[FrontendSessionService][fetchAll] No data found in session. Method: ${request.method}, req: ${request.path}, param: ${request.rawQueryString}"
        logger.warn(errMsg)
        Future.failed(new NoSuchElementException(errMsg))
    } recoverWith { case e: Throwable =>
      val errMsg =
        s"[FrontendSessionService][fetchAll] Error fetching all keys. Method: ${request.method}, req: ${request.path}, param: ${request.rawQueryString}, Exception: ${e.getMessage}"
      logger.error(errMsg, e)
      Future.failed(new Exception(errMsg))
    }

  def getAltAmmendsData(schemeRef: String)(implicit
    ec: ExecutionContext,
    request: RequestHeader
  ...
  def getGroupSchemeData(schemeRef: String)(implicit
    request: RequestHeader,
    ec: ExecutionContext
  ): Future[(Option[GroupSchemeInfo], Option[CompanyDetailsList])] =
    for {
  ...
  def getAllData(bundleRef: String, ersMetaData: ErsMetaData)(implicit
    ec: ExecutionContext,
    ...
    (for {
      ...
      ErsSummary(
        ...
        altAmendsActivity = altData._1,
        alterationAmends = altData._2,
        groupService = gc._1,
        schemeOrganiser = soc,
        companies = gc._2,
        trustees = td,
      ...
      logger.error(
        s"[FrontendSessionService][getAllData]: Get all data from cache failed with exception ${e.getMessage}, timestamp: ${System.currentTimeMillis()}.",
        e
      )
      throw e
    }
  ...
  def getNoOfRows(nilReturn: String)(implicit
    ec: ExecutionContext,
    request: RequestWithOptionalAuthContext[AnyContent],
    hc: HeaderCarrier
  ): Future[Option[Int]] =
  ...
  def fetchPartFromCompanyDetailsList[A](
    index: Int
  )(implicit request: RequestHeader, formats: json.Format[A]): Future[Option[A]] =
    sessionCache.getFromSession[JsValue](DataKey(SUBSIDIARY_COMPANIES_CACHE)).map { subsidiaryCompanies =>
      subsidiaryCompanies.map(_.\(COMPANIES).as[JsArray].\(index).getOrElse(Json.obj()).as[A])
    } recover { case x: Throwable =>
      logger.debug(
        "[ERSUtil][fetchPartFromCompanyDetailsList] Nothing found in cache, expected if this is not an edit journey: " + x.getMessage
      )
      None
    }

  def fetchPartFromCompanyDetails[A]()(implicit request: RequestHeader, formats: json.Format[A]): Future[Option[A]] =
    sessionCache.getFromSession[JsValue](DataKey(SCHEME_ORGANISER_CACHE)).map { companyDetailsOpt =>
  ...
  def fetchCompaniesOptionally()(implicit
    request: RequestHeader,
    formats: json.Format[CompanyDetailsList]
  ): Future[CompanyDetailsList] =
    fetch[CompanyDetailsList](SUBSIDIARY_COMPANIES_CACHE).recover { case _ =>
      CompanyDetailsList(List.empty[CompanyDetails])
    }

  def fetchSchemeOrganiserOptionally()(implicit
    request: RequestHeader,
    formats: json.Format[CompanyDetails]
  ): Future[Option[CompanyDetails]] =
    fetch[CompanyDetails](SCHEME_ORGANISER_CACHE).map(Some(_)).recover { case _ =>
      None
    }
