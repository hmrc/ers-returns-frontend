...
class TrusteeBasedInUkController @Inject() (
...
) extends FrontendController(mcc) with WithUrlEncodedOnlyFormBinding with TrusteeBaseController[TrusteeBasedInUk] {
  ...
  def nextPageRedirect(index: Int, edit: Boolean = false)(implicit
    ...
    for {
      trusteeBasedInUk <- if (edit) {
                            sessionService.fetchTrusteesOptionally().map { trusteeDetailsList =>
                              TrusteeBasedInUk(trusteeDetailsList.trustees(index).basedInUk)
                            }
                          } else {
                            sessionService.fetch[TrusteeBasedInUk](cacheKey)
                          }
    } yield (trusteeBasedInUk.basedInUk, edit) match {
  ...
  def view(
    ...
    index: Int,
    trusteeBasedInUkForm: Form[TrusteeBasedInUk],
    edit: Boolean = false
