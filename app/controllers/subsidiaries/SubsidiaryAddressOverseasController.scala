...
class SubsidiaryAddressOverseasController @Inject() (
...
) extends FrontendController(mcc) with WithUrlEncodedOnlyFormBinding with SubsidiaryBaseController[CompanyAddress] {
  ...
  def nextPageRedirect(index: Int, edit: Boolean = false)(implicit
    ...
    if (edit) {
      ...
      companyDetailsService
        .updateSubsidiaryCompanyCache(index)
        .map(_ => redirect)
  ...
  def view(
    ...
    index: Int,
    companyAddressOverseasForm: Form[CompanyAddress],
    edit: Boolean = false
