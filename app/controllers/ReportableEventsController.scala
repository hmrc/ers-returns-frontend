...
class ReportableEventsController @Inject() (
  ...
  def reportableEventsPage(): Action[AnyContent] = authAction.async { implicit request =>
    ersSessionCacheService.fetch[RequestObject](ersSessionCacheService.ersRequestObject).flatMap { requestObj =>
  ...
  def updateErsMetaData(
    ...
    ersConnector.connectToEtmpSapRequest(requestObject.getSchemeReference).flatMap { sapNumber =>
      ersSessionCacheService.fetch[ErsMetaData](ersSessionCacheService.ersMetaData, requestObject.getSchemeReference).map { metaData =>
        ...
        ersSessionCacheService.cache(ersSessionCacheService.ersMetaData, ersMetaData, requestObject.getSchemeReference).recover { case e: Exception =>
          logger.error(
  ...
  def showReportableEventsPage(
    ...
    ersSessionCacheService.fetch[ReportableEvents](ersSessionCacheService.reportableEvents, requestObject.getSchemeReference).map { activity =>
  ...
  def showReportableEventsSelected(
    ...
    RsFormMappings
      ...
      .fold(
        ...
        formData =>
          ersSessionCacheService.cache(ersSessionCacheService.reportableEvents, formData, requestObject.getSchemeReference).map { _ =>
            if (formData.isNilReturn.get == ersSessionCacheService.OPTION_NIL_RETURN) {
