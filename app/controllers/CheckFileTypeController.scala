...
class CheckFileTypeController @Inject() (
  ...
  def showCheckFileTypePage()(implicit
    ...
    (for {
      ...
      fileType      <- ersSessionCacheService.fetch[CheckFileType](ersSessionCacheService.FILE_TYPE_CACHE, requestObject.getSchemeReference).recover {
        ...
        .fold(
          ...
          formData =>
            ersSessionCacheService
              .cache(ersSessionCacheService.FILE_TYPE_CACHE, formData, requestObject.getSchemeReference)
              .map { _ =>
                if (formData.checkFileType.contains(ersSessionCacheService.OPTION_ODS)) {
