...
class ConfirmationPageController @Inject() (
  ...
  def showConfirmationPage()(implicit
    ...
    ersSessionCacheService.fetch[RequestObject](ersSessionCacheService.ersRequestObject).flatMap { requestObject =>
      ...
      if (sessionBundleRef == "") {
        ersSessionCacheService.fetch[ErsMetaData](ersSessionCacheService.ersMetaData, schemeRef).flatMap { all =>
          ...
          val submissionJson = ersSessionCacheService.getSubmissionJson(
            all.schemeInfo.schemeRef,
            all.schemeInfo.schemeType,
          ...
          ersConnector.connectToEtmpSummarySubmit(all.sapNumber.get, submissionJson).flatMap { bundle =>
            ersSessionCacheService.getAllData(bundle, all).flatMap { alldata =>
              if (alldata.isNilReturn == ersSessionCacheService.OPTION_NIL_RETURN) {
                ...
                ersSessionCacheService.fetch[String](ersSessionCacheService.VALIDATED_SHEEETS, schemeRef).flatMap { validatedSheets =>
        ...
        ersSessionCacheService.fetch[ErsMetaData](ersSessionCacheService.ersMetaData, schemeRef).flatMap { all =>
          ...
          Future(
            Ok(confirmationView(requestObject, sessionDateTimeSubmitted, sessionBundleRef, all.schemeInfo.taxYear, url))
          )
        }
      }
    ...
    ersConnector.saveMetadata(alldata).flatMap { res =>
      res.status match {
        case OK =>
          ...
          if (alldata.transferStatus.contains(ersSessionCacheService.largeFileStatus)) {
            None
          } else {
            ersConnector.submitReturnToBackend(alldata).map { response =>
              response.status match {
                case OK =>
                  ...
                  ersSessionCacheService.submitReturnToBackend(System.currentTimeMillis() - startTime, TimeUnit.MILLISECONDS)
                  logger.info(
