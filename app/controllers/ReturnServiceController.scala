...
class ReturnServiceController @Inject() (
  ...
  def cacheParams(ersRequestObject: RequestObject)(implicit request: Request[_], hc: HeaderCarrier): Future[Result] = {
    ...
    val futureResult = for {
      ...
      _ <- ersSessionCacheService.cache(ersUtil.ersMetaData, ersRequestObject.toErsMetaData, ersRequestObject.getSchemeReference)
      _ <- ersSessionCacheService.cache(ersUtil.ersRequestObject, ersRequestObject)
    } yield {
  ...
  def startPage(): Action[AnyContent] = authActionGovGateway.async { implicit request =>
    ersSessionCacheService.fetch[RequestObject](ersUtil.ersRequestObject).map { result =>
